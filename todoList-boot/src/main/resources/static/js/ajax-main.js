...
function getTotalCount() { // 함수 정의
    ...
    // 비동기로 서버에서 전체 Todo 개수 조회하는 fetch() API 코드 작성
    // fetch : 가지고 오다
...
reloadBtn.addEventListener('click', () => {
    ...
    getCompleteCount(); // 비동기로 완료 일 개수 조회
});
...
const selectTodo = (url) => {
    ...
    // response.json() : 응답 데이터가 JSON인 경우 이를 자동으로 Object 형태로 변환하는 함수
    // == JSON.parse(JSON 데이터)
    fetch(url)
...
const selectTodoList = () => {   // 변수에 함수를 대입하는 형식
    ...
    .then(result => { // result == 첫 번째 then에서 반환된 결과값
        ...
        // JSON은 객체가 아니라 문자열이구나~!
        // 문자열은 가공 가능하나 힘들다 -> JSON.parse(JSON데이터)
...
deleteBtn.addEventListener("click", () => {
    ...
    fetch("/ajax/delete", {
        ...
        body : todoNo // todoNo 값을 body에 담아서 전달 -> @RequestBody로 꺼냄
    })

    .then(response => response.text()) // 요청 결과 text 형태로 변환
    .then(result => {
...
changeComplete.addEventListener('click', e => {
    ...
    .then(result => {
        if(result > 0) {
            ...
            selectTodoList(); // 서버 부하 줄이기 가능 => 코드가 조금 복잡
        } else {
            alert("변경 실패");
        }
    });
...
updateView.addEventListener("click", () => {
    ...
    // HTML 화면에서 줄바꿈이 <br>로 인식되고 있는데 textarea에서는 \n으로 바꿔야 줄바꿈 인식 됨
...
// 수정 레이어에서 취소버튼(#updateCancel) 클릭 되었을 때
