...
const getCookie = key => {
    ...
    return obj[key]; // 매개 변수로 전달 받은 key와 obj 객체에 저장된 키가 일치하는 요소의 값 반환
...
if(loginEmail != null) {  // 로그인창의 이메일 입력 부분이 있을 때
    ...
    const saveId = getCookie("saveId"); // undefined 또는 email
...
// #loginForm 이 화면에 존재할 때 (== 로그인 상태 아닐때)
if(loginForm != null) {
