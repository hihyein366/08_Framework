/* querySelector() 문제점
   querySelector(), querySelectorAll()은 호출되었을 시점의 요소 형태를 그대로 얻어옴

   getElementsByClassName() 같은 경우에는
   요소를 얻어와 계속 추적 -> 실시간으로 변경된 값 확인 가능
*/
...
// 이미지 선택 후 취소를 누를 경우를 대비한 백업 이미지
...
const changeImageFn = (inputImage, order) => {
    ...
    // ----- 선택된 파일의 크기가 최대 크기(maxSize) 초과한 경우 ------

    if(file.size > maxSize) {
        alert("10MB 이하의 이미지를 선택해 주세요");

        // 해당 순서의 backup 요소가 없거나, 요소는 있는데 값이 없는 경우 
        // == 아무 파일도 선택된 적 없을 때
        if(backupInputList[order] == undefined
            || backupInputList[order].value == ''){
            inputImage.value = ""; // 잘못 업로드 된 파일 값 삭제
            return;
        }
...
for(let i=0; i<inputImageList.length; i++) {
    ...
    deleteImageList[i].addEventListener("click", () => {
        ...
        // img, input, backup 의 인덱스가 모두 일치한다는 특징을 이용

        previewList[i].src = "";  // 미리보기 사진 제거
