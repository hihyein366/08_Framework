...
- 자원(데이터, 파일 응답)을 이름(주소)으로 구분(representational)하여
  ...
  삽입 == POST / 조회 == GET / 수정 == PUT / 삭제 == DELETE
...
const selectCommentList = () => {
    ...
    // boardDetail.html 밑에 보면 전역변수로 boardNo 가져와서 바로 사용가능
    ...
    .then(commentList => {
        ...
        // 조회된 commentList 이용해서 새로운 댓글 목록 출력

        // ul태그(댓글 목록 감싸는 요소)
        const ul = document.querySelector("#commentList");
        ul.innerHTML = ""; // 기존 댓글 목록 삭제 (inner는 시작/종료 태그 사이 ""으로 하겠다)

        /* ***** 조회된 commentList 이용해 댓글 출력 ***** */
        for(let comment of commentList){
            ...
            if(comment.commentDelFl == 'Y') commentRow.innerText = "삭제된 댓글 입니다";
                ...
                if(comment.profileImg == null) profileImg.src = userDefaultImage; // 기본 이미지
                ...
                // 작성자 영역(commentWriter)에 프로필 이미지, 날짜 추가하는 코드 작성
...
addContent.addEventListener("click", e => {
    ...
    const data = {
        ...
        "boardNo" : boardNo, // boardDetail.html 에 가져와 있음
