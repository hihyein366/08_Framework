...
// 프로필 이미지가 새로 업로드 되거나 삭제 되었음을 기록하는 상태변수
...
// 요소. cloneNode(true|false) : 요소 복제(true 작성 하위 요소도 복제)
let backupInput;

if(profile != null) {

    // img 태그 (프로필 이미지 보여지는 요소)
    const profileImg = document.querySelector("#profileImg");
    
    // input type="file" 태그 (실제 업로드할 프로필 이미지를 선택하는 요소)
    let imageInput = document.querySelector("#imageInput");
    
    // x 버튼 (프로필 이미지 제거 후 기본으로 변경 할 요소)
    ...
    const changeImageFn = e => {
        ...
        if(file == undefined) {
            ...
            // 파일 선택 후 취소 -> value == ''
            // -> 선택한 파일 없음으로 기록됨
            // -> backupInput으로 교체시켜서 이전 이미지가 남아 있는 것 처럼 보이게 함
            ...
            // 화면에 추가된 백업본에는 이벤트 리스너가 존재하지 않기 때문에 추가하겠다
            imageInput.addEventListener("change", changeImageFn);

            // 한번 화면에 추가된 요소(backupInput)는 재사용 불가
        ...
        if(file.size > maxSize) {
            ...
            if(statusCheck == -1){
                imageInput.value = '';
            } else { // 기존 선택된 이미지가 있는데 다음 선택 이미지가 최대 크기 초과한 경우
        ...
        // 선택한 파일(file) 읽어와서 BASE64 인코딩 형태로 읽어와 result변수에 저장
...
/* [input type="file" 사용 시 유의 사항]
    ...
    1. 파일 선택 후 취소를 누르면 선택 파일 사라짐 (value == '')
