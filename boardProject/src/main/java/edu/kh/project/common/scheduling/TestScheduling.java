 ...
 * 
 * 2. 스케줄러 코드를 작성할 별도 클래스를 생성한 후 Bean 으로 등록
 *   -> @Component 어노테이션 작성
 *   
 * 3. 해당 클래스에 @Scheduled(시간/주기) 어노테이션 추가한 메서드 작성
 *   -> 주의 사항 **
 *   - 해당 메서드는 반환형이 존재해서는 안된다! == void 
 */

@Component // bean 등록 -> 스프링이 자동으로 스케줄링 코드를 수행하게 함
@Slf4j
public class TestScheduling {

	// @Scheduled() 매개 변수
	
	// 1) fixedDelay : 이전 작업이 끝난 후 다음 작업이 시작할 때까지의 시간 지정.
	
	// 2) fixedRate : 이전 작업이 시작한 후 다음 작업이 시작할 때까지의 시간 지정.
	
	// 3) cron : UNIX 계열 잡 스케쥴러 표현식
	...
	// / : 값 증가. 	  ex) 0/5 => 0부터 시작해서 5씩 증가할때 마다 수행
	// ? : 특별한 값 없음 (월 / 요일만 가능)
	// L : 마지막 (월 / 요일만 가능) Last.
...
//	@Scheduled(cron="59 59 23 * * *") // 다음날 되기 1초전
