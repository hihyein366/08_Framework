...
/* Pagination 뜻 : 목록을 일정 페이지로 분할해서 원하는 페이지 볼수 있게 하는 것
 * 				  == 페이징 처리
 * 
...
public class Pagination {
	...
	private void calculate() {
		...
		// 페이지 번호 목록이 10개씩 보여질 경우 (pageSize)
		...
		// nextPage : ">" 클릭 시 이동할 페이지 번호 (다음 페이지 번호 목록 중 시작번호)
		
		// 더 이상 넘어갈 페이지 없을 경우 (다음 페이지 번호 목록 중 시작 번호)
		if(endPage == maxPage) {
			nextPage = maxPage;
		} else { 
			nextPage = endPage + 1;
