...
@Transactional // 예외 발생하면 롤백한다 (기본값 커밋)
@Service // Bean 등록
...
public class EmailServiceImpl implements EmailService {
	...
	// 타임리프(템플릿 엔진) 이용해서 html 코드 -> java 변환
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			// MimeMessageHelper : Spring에서 제공하는 메일 발송도우미(간단 + 타임리프)
		...
		// 1) 해당 이메일이 DB에 존재하는 경우가 있을 수 있기 때문에 수정을 먼저 진행
		//	 -> 수정(update) 시 1 반환 == 업뎃 성공 == 이미 존재해서 인증번호 변경
		//	 -> 0 반환 == 업뎃 실패 == 이메일 존재 X
		//	  --> INSERT 시도
	...
	// HTML 파일을 읽어와 String으로 변환 (타임리프 적용되어 있음)
	public String loadHtml(String authKey, String htmlName) {
		...
		// templates/email 폴더에서 
		// htmlName과 같은 .html 파일 내용 읽어와 String으로 변환
 ...
 *
 * - Simple Mail Transfer Protocol, 간단한 메일 전송 규약
 *  --> 이메일 메시지 보내고 받을 때 사용하는 약속(규약, 방법)
 *  
 *  - Google SMTP
 *  
 *  Java Mail Sender -> Google SMTP -> 대상에게 이메일 전송
 *  
 * - Java Mail Sender에 Googld SMTP 이용 설정 추가
 * 1) config.properties
 * 2) EmailConfig.java
 */